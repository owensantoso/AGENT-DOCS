...
For a project that already has older AGENT-DOCS files, update the local Agent
...
| Missing manifest / legacy / manual review | Do not rerun `init --write`. If the project files are the intended local truth, run `agent-continuity baseline --dry-run /path/to/project --profile standard --docs-meta yes`, review the output, then use `--write` only if it matches the installed shape. |
| Missing or outdated AGENT-DOCS-owned tooling | Run `agent-continuity upgrade --dry-run /path/to/project` first. If only owned tooling changes are proposed, use `agent-continuity upgrade --write --tooling-only /path/to/project`. |
| Generated views are stale | Add `--generated-views` only when the views are manifest-tracked and you want them regenerated by `scripts/docs-meta update`. |
