...
Repeated installs update `~/.agent-continuity`, refresh the `agent-continuity`,
...
Write mode also creates `.agent-continuity/manifest.json` with schema version 1. The
manifest records source metadata when Git can provide it, the selected canonical profile,
optional components such as `agent-continuity-docs`, installed files, and timestamps. It
checksums Agent Continuity-owned reusable tooling and records expected file modes;
...
`--dry-run` is the default. `--profile` is required, and `--docs` accepts
`auto`, `yes`, or `no`. Write mode creates only `.agent-continuity/manifest.json` and
refuses if a manifest already exists, Agent Continuity-owned tooling is missing,
...
For a project that already has older Agent Continuity files, update the local Agent
Continuity command first:

```bash
curl -fsSL https://raw.githubusercontent.com/owensantoso/agent-continuity/main/install.sh | bash -s -- --no-run
```

Then inspect each target project from anywhere:
...
| Result | Next Step |
|---|---|
| Healthy/current | Nothing else is required. |
| Missing manifest / legacy / manual review | Do not rerun `init --write`. If the project files are the intended local truth, run `agent-continuity baseline --dry-run /path/to/project --profile standard --docs yes`, review the output, then use `--write` only if it matches the installed shape. |
| Missing or outdated Agent Continuity-owned tooling | Run `agent-continuity upgrade --dry-run /path/to/project` first. If only owned tooling changes are proposed, use `agent-continuity upgrade --write --tooling-only /path/to/project`. |
| Generated views are stale | Add `--generated-views` only when the views are manifest-tracked and you want them regenerated by `agent-continuity docs update`. |
| Project-owned Markdown changed | Review manually. The upgrade command reports these files but does not overwrite them. |
| Refused / unknown / incompatible shapes | Stop and inspect the finding. The tool is deliberately conservative around symlinks, unsafe paths, unrecognized records, and local drift. |
...
Legacy installs without `.agent-continuity/manifest.json` are reported as
...
Tooling-only write mode is limited to deterministic Agent Continuity-owned tooling:
...
`.agent-continuity/manifest.json` last. Backups for touched existing files are written
under `.agent-continuity/backups/<timestamp>/`, with an `audit.json` file for the
...
initially `agent-continuity docs update`. Exit codes are `0` for healthy/current,
...
Install Agent Continuity document tooling when the target repo should mechanically manage IDs, generated registries, todos, docs links, safe doc moves, roadmap views, and docs health checks.
...
cp "$AGENT_CONTINUITY_SOURCE/scaffold/agent-instructions/"*.md ./docs/repo-health/agent-instructions/
```
...
cp "$AGENT_CONTINUITY_SOURCE/scaffold/skills/structured-docs-workflow/SKILL.md" ./skills/structured-docs-workflow/SKILL.md
```

This gives fresh implementation agents a fast path for state docs, plans, briefs, structured `TODO-*`, and Agent Continuity document tooling.
...
- generated views that are stale after deleting examples; regenerate with `agent-continuity docs update` when installed
...
```text
You are in a repo that uses the Agent Continuity workflow.
...
Do not implement from an implementation brief alone. Read its parent plan first. When document tooling is installed, use `agent-continuity docs check` and `agent-continuity docs check-links` before closeout.
