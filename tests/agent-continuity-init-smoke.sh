...
python3 - "$compat_tiny_target/.agent-continuity/manifest.json" <<'PY'
import json
import sys
...
mkdir -p "$manifest_conflict_target/.agent-continuity"
echo "{}" >"$manifest_conflict_target/.agent-continuity/manifest.json"
...
require_contains "$tmpdir/manifest-conflict.out" ".agent-continuity/manifest.json"
...
"$installer" "$meta_target" --profile standard --docs yes --write >"$tmpdir/meta-write.out"
require_file "$meta_target/scripts/agent-continuity-docs"
require_file "$meta_target/tests/agent-continuity-docs-smoke.sh"
require_absent "$meta_target/scripts/docs-meta"
require_absent "$meta_target/tests/docs-meta-smoke.sh"
require_file "$meta_target/.agent-continuity/manifest.json"
require_contains "$tmpdir/meta-write.out" "scripts/agent-continuity-docs"
python3 - "$meta_target/.agent-continuity/manifest.json" <<'PY'
import hashlib
import json
import pathlib
import sys
...
if manifest.get("optional_components") != ["agent-continuity-docs"]:
    raise SystemExit("Expected agent-continuity docs optional component")
records = {record["path"]: record for record in manifest.get("files", [])}
for path in ("scripts/agent-continuity-docs", "tests/agent-continuity-docs-smoke.sh"):
    record = records.get(path)
    if not record:
        raise SystemExit(f"Expected manifest record for {path}")
    if record.get("ownership") != "agent-continuity-owned":
        raise SystemExit(f"Expected {path} to be Agent Continuity-owned")
...
for path in ("docs/SPECS.md", "docs/DOCS-REGISTRY.md", "docs/TODOS.md"):
    ...
    if record.get("generator") != "agent-continuity docs update":
        raise SystemExit(f"Expected agent-continuity docs update generator for {path}")
    ...
    if "Generated by agent-continuity docs update" not in text:
        raise SystemExit(f"Expected canonical Agent Continuity marker in {path}")
    if "Generated by scripts/docs-meta update" in text:
        raise SystemExit(f"Fresh install must not emit legacy docs-meta marker in {path}")
    expected = hashlib.sha256(view_path.read_bytes()).hexdigest()
    if record.get("checksum_sha256") != expected:
        raise SystemExit(f"Expected generated view checksum for {path}")
PY

"$installer" "$tmpdir/meta-preview-app" --profile standard --docs yes --dry-run >"$tmpdir/meta-preview.out"
require_absent "$tmpdir/meta-preview-app/.agent-continuity/manifest.json"
python3 - "$tmpdir/meta-preview.out" <<'PY'
import sys

text = open(sys.argv[1], encoding="utf-8").read()
preview = text.split("Structure preview:", 1)[1].split("\n\nTarget:", 1)[0]
if "scripts/" not in preview or "agent-continuity-docs" not in preview:
    raise SystemExit("Expected explicit agent-continuity docs choice to appear in structure preview")
PY
...
EOF
if "$installer" "$meta_generated_conflict_target" --profile standard --docs yes --write >"$tmpdir/meta-generated-conflict.out" 2>&1; then
...
require_absent "$meta_generated_conflict_target/.agent-continuity/manifest.json"
require_absent "$meta_generated_conflict_target/AGENTS.md"
require_absent "$meta_generated_conflict_target/scripts/agent-continuity-docs"
...
docs_tooling_action_count="$(grep -Fc -- "Would create: scripts/agent-continuity-docs" "$tmpdir/full-dry-run.out")"
if [[ "$docs_tooling_action_count" != "1" ]]; then
  echo "Expected full profile dry run to create scripts/agent-continuity-docs once, got $docs_tooling_action_count" >&2
  exit 1
fi

"$installer" "$tmpdir/full-no-meta-app" --profile complete --docs no --dry-run >"$tmpdir/full-no-meta-dry-run.out"
require_contains "$tmpdir/full-no-meta-dry-run.out" "└── docs/"
if grep -Fq -- "tooling: scripts/agent-continuity-docs" "$tmpdir/full-no-meta-dry-run.out"; then
...
echo "agent-continuity init smoke test passed"
